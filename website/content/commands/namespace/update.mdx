...
- `-default-policy-id=<value>` - ID of a policy from the default namespace to inject for all tokens
  in this namespace. May be specified multiple times. Note that the ACL token used with
  this command must have `acl:write` access to the linked policy.

- `-default-policy-name=<value>` - Name of a policy from the default namespace to inject for all
  tokens in this namespace. May be specified multiple times. Note that the ACL token used with
  this command must have `acl:write` access to the linked role.
