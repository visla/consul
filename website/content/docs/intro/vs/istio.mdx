...
To enable the full functionality of Istio, control plane and data plane must
be deployed. For the control plane: istiod service must be
...
or others. Finally, Istio relies on Kubernetes Control Plane to store state,
which typically is etcd. Istio mainly requires running Kubernetes Cluster. 
